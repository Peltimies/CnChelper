...
function parseSpells(content: string): ParsedSpell[] {
  ...
  const isSpellHeader = (line: string): boolean => {
    // Look for spell name patterns followed by level info
  ...
  while (i < lines.length) {
    ...
    // Extract spell name and level info
    ...
    // Parse the rest of the header for spell attributes
    ...
    // Collect description lines until next spell header
    while (i < lines.length) {
...
function importSpells() {
  ...
  console.log(`Successfully imported ${spellsToAdd.length} new spells to database`);
  console.log('\nNew spells added:');
  spellsToAdd.forEach(spell => {
    console.log(`  - ${spell.name} (Level ${spell.level}, Classes: ${spell.classes.join(', ')})`);
  });
}
